...
def multistep_lr(args, batch_num):
    ...
    learning_rate = []
    total_steps = int(args.epochs * batch_num)

    def _lr_adjuster(step, warmup_step):
        if step < warmup_step:
            lr = _warmup_lr(args.warmup_lr, args.base_lr, warmup_step, step)
        else:
            lr = args.base_lr * (0.1 ** (int(step) // int(total_steps * 0.3)))
        return lr

    for step in range(int(args.epochs * batch_num)):
        learning_rate.append(_lr_adjuster(step, warmup_step=int(total_steps // 20)))
